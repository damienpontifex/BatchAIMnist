...
def cnn_model_fn(features, labels, mode, params):
    ...
    with tf.name_scope('Input'):
        # Input Layer
        input_layer = tf.reshape(features['image'], [-1, 28, 28, 1], name='input_reshape')
    ...
    if mode != tf.estimator.ModeKeys.PREDICT:
        ...
        correct_prediction = tf.equal(labels, predicted_logit)
...
def serving_input_receiver_fn():
    feature_spec = {
        'image': tf.FixedLenFeature(dtype=tf.float32, shape=[28*28])
    }
